...
export interface CalculationResult {
  rawReportedTotal: number;
  ramtalApprovedTotal: number;
  finalCalculatedMeals: number;
  finalTotalAmountNis: number;
  auditTrail: CalculationAuditEntry[];
  breakdownByMealType: Record<string, { qty: number; rate: number; totalNis: number }>;
}

export interface QuarterlyContext {
  isQuarterClosingMonth: boolean; // האם חודש 3, 6, 9 או 12
  quarterNumber: number;          // 1..4
  priorMonthsActualMeals: number; // כמות הארוחות בשני החודשים הקודמים של הרבעון
}
...
export class MealCalculationEngine {
  ...
  public static calculateMonthlySummary(
    ...
    const auditTrail: CalculationAuditEntry[] = [];
    ...
    } else if ((kitchen as any).initialTotalApproved) {
      rawReportedTotal = (kitchen as any).initialTotalReported || (kitchen as any).initialTotalApproved;
      ramtalApprovedTotal = (kitchen as any).initialTotalApproved;
    ...
    const breakdownByMealType: Record<string, { qty: number; rate: number; totalNis: number }> = {
      total: {
        qty: currentMeals,
        rate: defaultRate,
